...
app.post('/api/products', async (req, res) => {
  const { name, price, category, quantity, primary_unit, secondary_unit, conversion_rate } = req.body;
  if (!name || price === undefined) {
    return res.status(400).json({ error: 'Name and price are required' });
  }
  // quantity is in secondary units (pieces)
  const { data, error } = await supabase
    .from('products')
    .insert({
      name,
      ...
      category: category || '',
      quantity: parseInt(quantity) || 0, // stock in secondary units
      primary_unit: primary_unit || 'Box',
      secondary_unit: secondary_unit || 'Piece',
      conversion_rate: parseInt(conversion_rate) || 1,
    })
    .select();
  if (error) return res.status(500).json({ error: error.message });
  res.status(201).json(data[0]);
...
app.patch('/api/products/:id', async (req, res) => {
  ...
  if (primary_unit !== undefined) updates.primary_unit = primary_unit;
  if (secondary_unit !== undefined) updates.secondary_unit = secondary_unit;
  if (conversion_rate !== undefined) updates.conversion_rate = parseInt(conversion_rate);
  const { data, error } = await supabase
    .from('products')
    .update(updates)
    .eq('id', id)
...
app.post('/api/sales', async (req, res) => {
  const { productId, customerId, amount, quantity, orderId, unit } = req.body;
  ...
  const { data: product, error: prodErr } = await supabase
    .from('products')
    .select('price, quantity, primary_unit, secondary_unit, conversion_rate')
    .eq('id', productId)
    .single();
  if (prodErr) return res.status(500).json({ error: prodErr.message });
  if (!product) return res.status(404).json({ error: 'Product not found' });

  // Determine base quantity (in secondary units)
  let baseQty = qty;
  const selectedUnit = unit || product.secondary_unit;
  if (selectedUnit === product.primary_unit) {
    // Selling by primary unit → multiply by conversion_rate
    baseQty = qty * product.conversion_rate;
  } else {
    // Selling by secondary unit → qty as is
    baseQty = qty;
  }

  if (product.quantity < baseQty) {
    return res.status(400).json({ error: `Insufficient stock (need ${baseQty} ${product.secondary_unit}s)` });
  }

  // Calculate amount: if not provided, use price per secondary unit * baseQty
  let finalAmount = amount;
  if (!finalAmount) {
    const unitPrice = parseFloat(product.price); // price per secondary unit
    finalAmount = unitPrice * baseQty;
  }
  ...
  const { data: sale, error: saleErr } = await supabase
    ...
    .insert({
      ...
      amount: finalAmount,
      quantity: baseQty, // store base quantity in secondary units
      order_id: orderId || null,
    })
    .select(`
      *,
      customers (name, phone),
      products (name, price, primary_unit, secondary_unit, conversion_rate)
    `);
  if (saleErr) return res.status(500).json({ error: saleErr.message });

  // Update stock
  const newStock = product.quantity - baseQty;
  await supabase
    .from('products')
...
app.get('/api/sales/order/:orderId', async (req, res) => {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      *,
      customers (name, phone),
      products (name, price, conversion_rate, primary_unit, secondary_unit)
    `)
    .eq('order_id', orderId);
...
app.get('/api/dashboard/stats', async (req, res) => {
  ...
  try {
    ...
    const { data: products, error: s3 } = await supabase
      ...
      .select('price, quantity');
    if (s3) throw s3;
    const totalProductValue = products.reduce((acc, p) => acc + (p.price * (p.quantity || 0)), 0);
